...
def insert_sub_genre_assignment(conn: Connection,
    subscriber_id: int, genres_sub: List[str]) -> None:
    """
...
def form() -> Union[str, Response]:
    ...
    with connection.begin() as conn:
        genres = pd.read_sql(f"""SELECT * FROM genre where
        genre_name in {VALID_GENRES};""", conn)['genre_name']
